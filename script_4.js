const entrepreneurs = [
  { first: 'Steve', last: 'Jobs', year: 1955 },
  { first: 'Oprah', last: 'Winfrey', year: 1954 },
  { first: 'Bill', last: 'Gates', year: 1955 },
  { first: 'Sheryl', last: 'Sandberg', year: 1969 },
  { first: 'Mark', last: 'Zuckerberg', year: 1984 },
  { first: 'Beyonce', last: 'Knowles', year: 1981 },
  { first: 'Jeff', last: 'Bezos', year: 1964 },
  { first: 'Diane', last: 'Hendricks', year: 1947 },
  { first: 'Elon', last: 'Musk', year: 1971 },
  { first: 'Marissa', last: 'Mayer', year: 1975 },
  { first: 'Walt', last: 'Disney', year: 1901 },
  { first: 'Larry', last: 'Page', year: 1973 },
  { first: 'Jack', last: 'Dorsey', year: 1976 },
  { first: 'Evan', last: 'Spiegel', year: 1990 },
  { first: 'Brian', last: 'Chesky', year: 1981 },
  { first: 'Travis', last: 'Kalanick', year: 1976 },
  { first: 'Marc', last: 'Andreessen', year: 1971 },
  { first: 'Peter', last: 'Thiel', year: 1967 }
];
let entrepreneurs70 = [];


for(var i=0; i<entrepreneurs.length; i++) {
	if(entrepreneurs[i].year >= 1970 && entrepreneurs[i].year <= 1979) {
		
		entrepreneurs70.push(entrepreneurs[i]);
	}
}
console.log("\n #################Filtre dans cette liste les entrepreneurs qui sont nés dans les années 70");
console.log(entrepreneurs70);

let entrepreneursName = [];

entrepreneurs.forEach(entrepreneurs => {
  let entrepreneursInter = [];
  entrepreneursInter.push(entrepreneurs.first);
  entrepreneursInter.push(entrepreneurs.last);
  entrepreneursName.push(entrepreneursInter.join(' '));

});
console.log("\n ###############Sors une array qui contient le prénom et le nom des entrepreneurs");
console.log(entrepreneursName);

console.log("\n ###############Quel âge aurait chaque inventeur aujourd'hui ?");
entrepreneurs.forEach(entrepreneurs => {
  let ageOfEntrepreneur = 2021 - entrepreneurs.year
  console.log(`${entrepreneurs.first} ${entrepreneurs.last} a ou aurait ${ageOfEntrepreneur} ans aujourd'hui`) 
});
  

console.log("\n ################## Trie les entrepreneurs par ordre alphabétique du nom de famille.")
let sortedEntrepreneurs = entrepreneurs.sort(function(a,b) {
  return a.last.localeCompare(b.last);
});

console.log(sortedEntrepreneurs)